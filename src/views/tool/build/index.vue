<template>
    <div class="container">
        ...
        <code-type-dialog :visible.sync="dialogVisible" :title="$t('build.index.2090840-7')" :show-file-name="showFileName" @confirm="generate" />
...
export default {
    ...
    mounted() {
        const clipboard = new ClipboardJS('#copyNode', {
            text: (trigger) => {
                ...
                this.$notify({
                    ...
                    message: this.$t('build.index.2090840-8'),
                    type: 'success',
                });
                return codeStr;
            },
        ...
        clipboard.on('error', (e) => {
            this.$message.error(this.$t('build.index.2090840-9'));
        });
    },
    methods: {
        ...
        empty() {
            this.$confirm(this.$t('build.index.2090840-10'), this.$t('build.index.2090840-11'), { type: 'warning' }).then(() => {
                this.drawingList = [];
            });
        },
