  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item :label="$t('system.dict.data.879098-1')" prop="dictLabel">
        <el-input v-model="queryParams.dictLabel" :placeholder="$t('system.dict.data.879098-2')" clearable
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['system:dict:edit']">{{ $t('update') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['system:dict:remove']">{{ $t('del') }}</el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-close" size="mini"
          @click="handleClose">{{ $t('close') }}</el-button>
      </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        dictLabel: [
          { required: true, message: this.$t('system.dict.data.879098-23'), trigger: "blur" }
        ],
        dictValue: [
          { required: true, message: this.$t('system.dict.data.879098-24'), trigger: "blur" }
        ],
        dictSort: [
          { required: true, message: this.$t('system.dict.data.879098-25'), trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm(this.$t('system.dict.data.879098-28', [dictCodes])).then(function () {
