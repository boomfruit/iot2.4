  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      <el-form-item :label="$t('user.profile.index.894502-1')" prop="userName">
        <el-input v-model="queryParams.userName" :placeholder="$t('role.auth-user.876234-0')" clearable
          style="width: 240px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item :label="$t('user.index.098976-3')" prop="phonenumber">
        <el-input v-model="queryParams.phonenumber" :placeholder="$t('user.index.098976-4')" clearable
          style="width: 240px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="openSelectUser"
          v-hasPermi="['system:role:add']">{{ $t('role.auth-user.876234-1') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-circle-close" size="mini" :disabled="multiple"
          @click="cancelAuthUserAll" v-hasPermi="['system:role:remove']">{{ $t('role.auth-user.876234-2') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-close" size="mini" @click="handleClose">{{ $t('close')
          }}</el-button>
      </el-col>
      ...
      <el-table-column :label="$t('opation')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-circle-close" @click="cancelAuthUser(scope.row)"
            v-hasPermi="['system:role:remove']">{{ $t('role.auth-user.876234-3') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    cancelAuthUser(row) {
      ...
      this.$modal.confirm(this.$t('role.auth-user.876234-4') + row.userName + this.$t('role.index.094567-37')).then(function () {
        return authUserCancel({ userId: row.userId, roleId: roleId });
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess(this.$t('role.auth-user.876234-5'));
      }).catch(() => { });
    },
    /** 批量取消授权按钮操作 */
    cancelAuthUserAll(row) {
      ...
      this.$modal.confirm(this.$t('role.auth-user.876234-6')).then(function () {
