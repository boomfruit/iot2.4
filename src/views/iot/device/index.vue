            ...
            <el-table v-loading="loading" :data="deviceList" border>
                ...
                <el-table-column :label="$t('device.index.105953-34')" align="center" class-name="small-padding fixed-width" width="200">
                    <template slot-scope="scope">
                        ...
                        <el-button type="primary" size="small" style="padding: 5px" icon="el-icon-view" @click="handleEditDevice(scope.row)" v-hasPermi="['iot:device:query']">
                            {{ $t('device.index.105953-36') }}
                        </el-button>
                        <el-button type="primary" size="small" style="padding: 5px" @click="openSummaryDialog(scope.row)" v-if="form.deviceId != 0" v-hasPermi="['iot:device:query']">
