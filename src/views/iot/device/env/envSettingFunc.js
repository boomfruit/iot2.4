export function generateFanSettingTable(fans, level, valueIDs) {
    ...
    // 合并所有列：表头 + 风机列 + 结束列
    const settingcolumns = [...header, ..._columns, ...endcolumns];
    ...
    for (let j = 0; j < level; j++) {
        const row = {
            level: { key: 'level', value: j + 1, sort: 0 }, // 当前等级
        };

        // 初始化每个风机的状态为“关闭”
        for (let i = 0; i < settings.fans; i++) {
            row[`fan${i + 1}`] = { key: `fan${i + 1}`, value: '0', sort: i + 1 }; // 默认值为 "0"（关闭）
        }

        // 添加其他字段
        row.startTime = { key: 'startTime', value: '', sort: settings.fans + 1 };
        row.endTime = { key: 'endTime', value: '', sort: settings.fans + 2 };
        row.temperature = {
            key: 'temperature',
            value: '',
            sort: settings.fans + 3,
        };
    ...
    rows.forEach((item, idx) => {
        ...
        for (const key in item) {
            ...
            if (sort !== undefined && _arr[sort] !== undefined) {
                item[key].key = _arr[sort]; // 将 key 赋值给 arr 的对应位置
            }
        }
    });
