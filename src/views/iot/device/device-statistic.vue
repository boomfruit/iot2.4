      ...
      <el-col :span="24">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="75px">
          <el-form-item :label="$t('device.device-statistic.932674-0')" label-width="120px" v-if="isSubDev">
            <el-select v-model="queryParams.slaveId" :placeholder="$t('device.device-statistic.932674-1')"
              @change="selectSlave">
          ...
          <el-form-item :label="$t('device.device-statistic.932674-2')">
            <el-date-picker v-model="daterangeTime" size="small" style="width: 240px" value-format="yyyy-MM-dd"
              type="daterange" range-separator="-" :start-placeholder="$t('device.device-statistic.932674-3')"
              :end-placeholder="$t('device.device-statistic.932674-4')"></el-date-picker>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" icon="el-icon-search" size="mini" @click="getListHistory">{{
              $t('device.device-statistic.932674-5') }}</el-button>
          </el-form-item>
        </el-form>
      </el-col>
      <el-col :span="23">
    ...
    getStatistic() {
      ...
      for (let i = 0; i < this.staticList.length; i++) {
        ...
        option = {
          ...
          title: {
            left: 'center',
            text: this.staticList[i].name + '统计 （单位 ' + (this.staticList[i].datatype && this.staticList[i].datatype.unit != undefined ? this.staticList[i].datatype.unit : this.$t('device.device-statistic.932674-7')) + '）',
          },
          grid: {
            top: '80px',
            left: '40px',
          ...
          yAxis: {
            type: 'value',
            scale: true,//自适应
            axisLabel: { formatter: function (value, index) { return value.toFixed(2); } }//可根据实际情况修改y轴保留小数位数
          },
          dataZoom: [
            {
              type: 'inside',
